...
export const objectifyError = (object) => {
  ...
  Object.entries({ name, stack, cause, errors }).forEach(([propName, propValue]) => {
...
const getMessage = (message, object) =>
  typeof message === 'string' && message !== '' ? message : truncateMessage(safeJsonStringify(object));
...
const truncateMessage = (message) =>
  message.length < MESSAGE_MAX_SIZE ? message : `${message.slice(0, MESSAGE_MAX_SIZE)}...`;
